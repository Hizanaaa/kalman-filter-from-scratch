...
def run_experiment(
    ...
    angular_bias_drift_std,
    filter_velocity_variance,
    filter_angular_variance,
    num_steps=100,
):
    ...
    odometry = OdometrySensor(
        ...
        angular_velocity_noise_std=angular_noise_std,
        velocity_bias_drift_std=velocity_bias_drift_std,
        angular_velocity_bias_drift_std=angular_bias_drift_std,
        random_seed=seed,
    )
    ...
    ekf = OdometryEKF(
        ...
        velocity_variance=filter_velocity_variance,
        angular_velocity_variance=filter_angular_variance,
    )
...
def evaluate_condition(
    ...
    for seed in range(50):
        gps_rmse, ekf_rmse, improvement = run_experiment(
            seed=seed,
            velocity_noise_std=actual_noise[0],
            angular_noise_std=actual_noise[1],
            velocity_bias_drift_std=actual_noise[2],
            angular_bias_drift_std=actual_noise[3],
            filter_velocity_variance=filter_variances[0],
            filter_angular_variance=filter_variances[1],
        )
...
def main():
    print("\nControl-Noise Modeling Experiment")
    print("=================================")
    ...
    # Variances corresponding to the medium-noise sensor.
    medium_variances = (
        0.05 ** 2,
        0.01 ** 2,
    )

    # Variances corresponding to the high-noise sensor.
    high_variances = (
        0.15 ** 2,
        0.03 ** 2,
    )
    ...
    evaluate_condition(
        name="Medium Noise - Correctly Modeled",
        actual_noise=medium_noise,
        filter_variances=medium_variances,
    )

    evaluate_condition(
        name="High Noise - Filter Uses Medium Variance",
        actual_noise=high_noise,
        filter_variances=medium_variances,
    )

    evaluate_condition(
        name="High Noise - Correctly Modeled",
        actual_noise=high_noise,
        filter_variances=high_variances,
    )
