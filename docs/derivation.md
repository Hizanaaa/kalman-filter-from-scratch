...
This document derives the Linear Kalman Filter used in this project from the underlying robot motion and measurement models.
...
The robot is assumed to move with approximately constant velocity during one timestep.
...
$$ \begin{bmatrix} x_k \\ y_k \\ v_{x,k} \\ v_{y,k} \end{bmatrix} = \begin{bmatrix} x_{k-1}+v_{x,k-1}\Delta t \\ y_{k-1}+v_{y,k-1}\Delta t \\ v_{x,k-1} \\ v_{y,k-1} \end{bmatrix} $$
...
$$
x_k = 1x_{k-1} + 0y_{k-1} + \Delta t\,v_{x,k-1} + 0v_{y,k-1}
$$
...
$$
y_k = 0x_{k-1} + 1y_{k-1} + 0v_{x,k-1} + \Delta t\,v_{y,k-1}
$$
...
$$
v_{x,k} = 0x_{k-1} + 0y_{k-1} + 1v_{x,k-1} + 0v_{y,k-1}
$$
...
$$
v_{y,k} = 0x_{k-1} + 0y_{k-1} + 0v_{x,k-1} + 1v_{y,k-1}
$$
...
In a real system, the robot may accelerate, decelerate, or experience other unmodeled effects.
...
If $Q$ is too large, the filter may place too little confidence in its prediction.
...
The superscript $-$ means that the prediction has not yet incorporated the new measurement.
...
The innovation measures the disagreement between the actual GPS measurement and the predicted GPS measurement.
...
A large innovation indicates that the GPS measurement differs significantly from the predicted position.
...
Therefore, the filter automatically adjusts how much it trusts the GPS measurement.
...
Therefore, the Kalman gain provides an automatic weighting mechanism between prediction and measurement.

---

## 15. State Update

The corrected state is obtained by adding the weighted innovation to the predicted state:
...
The updated state lies between the prediction and the measurement, with the relative influence determined by the Kalman gain.

---

## 16. Covariance Update

After incorporating the measurement, the uncertainty should generally decrease.
...
The measurement provides additional information about the robot's position, so uncertainty in the state estimate is reduced.
...
This form explicitly preserves the contribution of measurement noise and can provide better numerical behavior in some implementations.
...
$$
\text{Prediction} \rightarrow \text{Measurement Update}
$$
...
$$
\text{Prediction} \rightarrow \text{Prediction} \rightarrow \text{Prediction} \rightarrow \cdots
$$
...
Instead, it is the expected representation of increasing uncertainty when absolute position measurements are unavailable.
...
$$ e_k = \left\| \begin{bmatrix} x_k^{estimate} \\ y_k^{estimate} \end{bmatrix} - \begin{bmatrix} x_k^{true} \\ y_k^{true} \end{bmatrix} \right\| $$
...
A good estimator should ideally maintain uncertainty that reflects the actual reliability of its estimate.
...
$$
1102.0521 \rightarrow 1.8278
$$

At the same time, the position error decreases substantially.

This experimentally demonstrates the relationship between measurement availability, uncertainty propagation, and state correction.
...
$$
\boxed{
\text{Predict} \rightarrow \text{Compare} \rightarrow \text{Weight} \rightarrow \text{Correct}
}
$$

The prediction step uses the robot's motion model.

The covariance prediction represents uncertainty introduced by the imperfect motion model.

The measurement step introduces information from GPS.

The Kalman gain determines the relative influence of prediction and measurement.

The update step produces the corrected state and reduced uncertainty.

This mathematical framework forms the foundation for the Extended Kalman Filter used later in the project.
